		...
		const load = async () => {
			...
			try {
				...
				const filtered = list.filter((a) => {
					// try common fields and also type keys to distinguish clinical vs medical
					const isForPatient = a.patientId === patientId || a.pacienteId === patientId || a.ownerId === patientId || (a.meta && (a.meta.patientId === patientId || a.meta.pacienteId === patientId));
					const isClinical = !a.type || String(a.type).toLowerCase().includes('clinical') || (a.meta && String(a.meta.category || '').toLowerCase().includes('clinical'));
					return isForPatient && isClinical;
				});
	...
	return (
		<main className="container mx-auto px-4 py-6">
			<div className="space-y-4">
				...
				<p className="text-sm text-muted-foreground">Registros clínicos y notas. Sólo vista.</p>

				<Card>
					<CardHeader><CardTitle>Registros</CardTitle></CardHeader>
					<CardContent>
						{loading ? <p>Cargando...</p> : attachments.length === 0 ? <p className="text-sm text-muted-foreground">No hay registros clínicos disponibles.</p> : (
									...
									<TableBody>
										{attachments.map((a) => (
											<TableRow key={a.id || a.name}>
												<TableCell className="font-medium">{a.name || a.filename || `Registro ${a.id}`}</TableCell>
				...
				<Dialog open={open} onOpenChange={setOpen}>
					<DialogContent className="sm:max-w-[900px]">
						<DialogHeader><DialogTitle>Registro</DialogTitle></DialogHeader>
						{selected ? (
							<div>
								{selected.url ? (
