  ...
  const fetch = async () => {
    try {
      ...
      if (user?.rol === "PACIENTE") {
        ...
        data = await CitasApi.getMine();
      } else {
        // ADMIN o MÉDICO → todas
        data = await CitasApi.getAll();
      }
  ...
  return (
    <main className="container mx-auto px-4 py-6">
          ...
          <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
            ...
            <DialogContent className="sm:max-w-[600px]">
              ...
              <NewCitaForm 
                ...
                cita={selected ?? undefined}
                isPaciente={user?.rol === "PACIENTE"} // <-- ayuda al form
              />
            </DialogContent>
          </Dialog>
        </div>
      ...
      <Dialog open={isEditOpen} onOpenChange={(open) => { 
        if (!open) { setIsEditOpen(false); setSelected(null); } 
      }}>
        <DialogContent className="sm:max-w-[600px]">
